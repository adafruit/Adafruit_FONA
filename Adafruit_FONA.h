...
typedef mraa::Uart FONAStreamType;
...
class Adafruit_FONA {
 public:
  ...
  boolean setBaudrate(int32_t baud);
  ...
  // boolean enableNTPTimeSync(boolean onoff, std::string ntpserver=0);
  ...
  // void setGPRSNetworkSettings(std::string apn, std::string username=0, std::string password=0);
  ...
  // boolean HTTP_init();
  // boolean HTTP_term();
  // void HTTP_para_start(std::string parameter, boolean quoted = true);
  // boolean HTTP_para_end(boolean quoted = true);
  // boolean HTTP_para(std::string parameter, const char *value);
  // boolean HTTP_para(std::string parameter, std::string value);
  // boolean HTTP_para(std::string parameter, int32_t value);
  ...
  // boolean HTTP_POST_start(char *url, std::string contenttype, const uint8_t *postdata, uint16_t postdatalen,  uint16_t *status, uint16_t *datalen);
 ...
 protected:
  ...
  uint8_t getReply(std::string send, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);
  // uint8_t getReply(std::string prefix, char *suffix, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);
  uint8_t getReply(std::string prefix, int32_t suffix, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);
  // uint8_t getReply(std::string prefix, int32_t suffix1, int32_t suffix2, uint16_t timeout); // Don't set default value or else function call is ambiguous.
  uint8_t getReplyQuoted(std::string prefix, std::string suffix, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);

  // boolean sendCheckReply(std::string prefix, char *suffix, std::string reply, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);
  boolean sendCheckReply(std::string prefix, int32_t suffix, std::string reply, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);
  // boolean sendCheckReply(std::string prefix, int32_t suffix, int32_t suffix2, std::string reply, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);
  boolean sendCheckReplyQuoted(std::string prefix, std::string suffix, std::string reply, uint16_t timeout = FONA_DEFAULT_TIMEOUT_MS);


  boolean parseReply(std::string toreply,
          uint16_t *v, char divider  = ',', uint8_t index=0);
  // boolean parseReply(std::string toreply,
  //         char *v, char divider  = ',', uint8_t index=0);
  boolean parseReplyQuoted(std::string toreply,
          char *v, int maxlen, char divider, uint8_t index);

  boolean sendParseReply(std::string tosend, std::string toreply, uint16_t *v, char divider = ',', uint8_t index=0);
  boolean sendParseReply(std::string tosend, std::string toreply, float *f,
    char divider = ',', uint8_t index=0);
  boolean parseReply(std::string toreply, float *f, char divider,uint8_t index);
...
class Adafruit_FONA_3G : public Adafruit_FONA {
  ...
  public:
    Adafruit_FONA_3G (int8_t r) : Adafruit_FONA(r) { _type = FONA3G_E; }
 ...
 protected:
   ...
   boolean HTTPS_request(std::string request);
